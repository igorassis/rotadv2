...
export const getRoutes = async (route) => {
    return await axios.get(baseUrl+`?apiKey=${API_KEY}&mode=fastest;truck&waypoint0=geo!${route.from_lat},${route.from_long}&waypoint1=geo!${route.to_lat},${route.to_long}&language=pt-br&routeattributes=sh,legs,bb,gr`).then(
        (res) => {
            return res;
        }
    ).catch((err) => {
